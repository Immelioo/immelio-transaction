...
const menuItems = [
  ...
  {
    label: "Demande de Recherche",
    href: "/pro/demande-recherche",
...
export default function ProSidebar() {
  ...
  const { isAdmin } = useProUser(false);
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.label}
                {item.href === "/pro/messages" && <MessagesBadge />}
              </Link>
            );
          })}
        </nav>
        ...
        {isAdmin && (
          <div className="px-4 pt-2">
            <Link
              ...
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-accent bg-accent/10 hover:bg-accent/20 transition-colors"
            >
              <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
